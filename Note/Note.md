...
二分算法基于二分查找，二分查找是在**有序数据集**中查找指定数据，查找的数据是确定的，且查找的结果是查找成功或查找失败，而二分算法最终的结果是一个数字，查找的数字只有一个范围，并不是确定的。二分算法本质是猜答案，与暴力类似，均为带入所猜测答案，计算结果判断是否符合预期，而二分算法是有策略的暴力算法。

#### 算法条件/算法背景

二分算法适用于求最值的题目，如果题目是最小化最大值、最大化最小值，那么一定使用二分算法

#### 算法过程

1. 判断题目中提供的$tag$，即$mid$经过计算得到的结果比较的对象，并确定答案范围$[l, r]$
2. 计算$mid$，$mid$代入计算函数得到结果，结果与$tag$比较
3. 根据比较结果，记录答案$ans$，并更新，如果$[l, r]$不合法或$l$与$r$重合，那么二分结束，输出答案，反之，继续执行2
...
>- $mid = (r - l) / 2 + l$
>
>上述两种计算方法虽然结果相同，但是对于方法一，如果$l$与$r$的数值比较大，那么其和会溢出，而方法二没有这个缺点，$mid$计算方法使用方法二即可

#### 算法时间复杂度

每个题目的$mid$计算方法不同，while循环内部时间复杂度不确定，而外部复杂度为$O(r - l)$
